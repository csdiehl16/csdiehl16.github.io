...
To get back to that natural process of inquiry, I created an AI assistant for exploratory data analysis and prototyping visualizations. Here were my requirements:
...
- Get a natural language query and turn it into a SQL, Javascript or Python expression
...
I created different functions for different operations on the data. My first version just uploaded JSON files and stored them in memory as Javascript arrays. I used methods from the [D3.js library](https://d3js.org/) to do things like sort, summarize and filter. I used few-shot prompting to help it decide which tool to use, and what parameters to include.
...
Then, in the system prompt, you can instruct the LLM to use it like this:
...
Taking this route meant that I needed a database, even if I didn’t intend to persist the data beyond a single session.
...
In fact, another (nice-looking and faster) version of what I did goes this route. You should [check this out](https://www.duckbook.ai/) as well! I used that developer’s [open-source React bindings](https://github.com/holdenmatt/duckdb-wasm-kit) for DuckDB-WASM to load a database in the browser, upload csv files and make queries.

But the catch was that I couldn’t access the client-side DB from the server, where all the AI action was happening. Although the chart component renders on the client side, the logic for when to render it lives in the server component, so I couldn’t just pass the DB in as a prop.
...
To ensure success, I included some critical guidance in the system prompt. The LLM is helpless without the database schema. Also, it helps to include a few sample rows. Think of what you would do as a human data analyst - the first instinct is probably to check the data types and column names, and peek at a few rows.
...
Of course, this structured approach limits the LLM’s creativity. Ideally, I wanted to let it generate its own chart code from scratch. It did a great job generating code, but I as of right now, haven’t found a great way to convert the string that it returns into executable code. I tried using [Javascript’s eval()](https://www.w3schools.com/jsref/jsref_eval.asp) and function constructor, which takes a string argument. However this is a dangerous, some say “evil” function, that can easily crash the application or create a security vulnerability.
...
Finally, added some nice UI features to make corrections when LLM goes wrong, such as flipping the chart, and showing the query. I aim to make data exporting and importing as seamless as possible. You can bring data from a csv or JSON at a URL or on your computer, and export the LLM’s summaries as csv.

## Future goals

This is just the beginning. One important feature I hope to implement next is follow-up questions. In the Vercel AI SDK you can keep track of message history in a shared state between AI and the client side, and feed this back into the model prompt on every call. With the data analysis it’s a bit more complex because I need to direct the LLM to find and use the correct summarized dataset from the last relevant prompt.
